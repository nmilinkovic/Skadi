...
class ReflectionUtilsTest {
  ...
  @Test
  def testFindSetter {
    val scalaMethod = ReflectionUtils.findSetter('scalaVariable, classOf[String], classOf[ClassWithSetters])
    Assert.assertTrue(scalaMethod.isDefined)

    val javaMethod = ReflectionUtils.findSetter('javaVariable, classOf[String], classOf[ClassWithSetters])
    Assert.assertTrue(javaMethod.isDefined)

    val noSuchMethod = ReflectionUtils.findSetter('noSuchVariable, classOf[String], classOf[ClassWithSetters])
